namespace TrotiNet
{
    ...
    public abstract class AbstractProxyLogic
    {
        ...
        protected void Connect(string hostname, int port)
        {
            ...
            log.DebugFormat("SocketPS created: [S{0}] {1} -> {2}", SocketPS.id, socket.LocalEndPoint, socket.RemoteEndPoint);
        }
    ...
    public class BaseProxyLogic: AbstractProxyLogic
    {
        ...
        /// <summary>
        /// Send proxy connection accepted to the client
        /// </summary>
        virtual protected void SendAcceptResponse()
        {
        ...
        /// </summary>
        virtual protected void HandleConnectEstablishConnection(string targetHost, int targetPort)
        {
            if (RelayHttpProxyHost == null)
            {
        ...
        virtual protected void HandleConnect()
        {
            ...
            HandleConnectEstablishConnection(NewDestinationHost, NewDestinationPort);
            log.DebugFormat("Connections opened: [S{0}] -> [S{1}]", this.SocketBP.id, this.SocketPS.id);

            this.State.NextStep = null;

            var cancellation = new CancellationTokenSource();
            var cancelToken = cancellation.Token;
            var downStreamTask = Task.Run(() => { this.SocketPS.TunnelDataTo(this.SocketBP, cancelToken); }, cancelToken);
            var upStreamTask = Task.Run(() => { this.SocketBP.TunnelDataTo(this.SocketPS, cancelToken); }, cancelToken);
            Task[] tasks = new Task[] { upStreamTask, downStreamTask };
            Task.WaitAny(tasks);

            // Close the sockets if they are not yet dead
            log.DebugFormat("Closing connections");
            cancellation.Cancel();
            Task.WaitAll(tasks, TimeSpan.FromSeconds(60));
            this.SocketBP.CloseSocket();
            ...
            {
                log.WarnFormat("Failed closing connections: {0}", e);
            }
        }
